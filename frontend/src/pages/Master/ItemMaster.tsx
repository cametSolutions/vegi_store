...
import { ItemSchema, ItemFormData } from "../../validation/itemmasterSchema"
...
const ItemMaster: React.FC = () => {
  ...
  const allbranches = useSelector(
    (state: RootState) => state.companyBranch.branches
  )
  ...
  const handleSave = async () => {
    ...
    if (!result.success) {
      ...
      result.error.issues.forEach((err) => {
        const fieldName = err.path[0] as keyof ItemFormData
