...
interface Field {
  name: keyof AccountmasterFormData
  label: string
  type: string
  placeholder?: string
  validation?: any
...
const AccountMaster = () => {
  ...
  const [formData, setFormData] = useState<AccountmasterFormData>({
    accountName: "",
    accountType: "",
    pricelevel: "",
    openingBalance: "",
  ...
  const [selectedHolder, setselectedHolder] = useState<AccountHolder | null>(null)
  const [filterType, setFilterType] = useState("")
  const [isEditing, setIsEditing] = useState(false)
  const [pricelevelOptions, setpricelevelOptions] = useState<Pricelevel[]>([])
  const [editingId, setEditingId] = useState<number | null>(null)
  const [submitSuccess, setSubmitSuccess] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false)
  const selectedCompany = useSelector(
    (state: RootState) => state.companyBranch.selectedCompany
  )
  const selectedBranch = useSelector(
    (state: RootState) => state.companyBranch.selectedBranch
  )
...
console.log("formdata",formData)
  ...
  const validateField = (name: keyof AccountmasterFormData, value: string) => {
    try {
