export const transactionTypes = [
  ...
  { value: 'purchase', label: 'Purchase Invoice', icon: 'TrendingDown', color: 'green' },
  { value: 'credit_note', label: 'Credit Note', icon: 'TrendingUp', color: 'emerald' },
  { value: 'debit_note', label: 'Debit Note', icon: 'RefreshCw', color: 'orange' }
];
...
export const getTransactionTypeByValue = (value) => {
  return transactionTypes.find(t => t.value === value);
};

export const getPartyLabel = (transactionType) => {
  return transactionType === 'sale' || transactionType === 'credit_note' ? 'Customer' : 'Supplier';
};

export const getDocumentLabel = (transactionType) => {
  switch(transactionType) {
    case 'sale': return 'Invoice No';
    case 'purchase': return 'Bill No';
    case 'credit_note': return 'Credit Note No';
    case 'debit_note': return 'Debit Note No';
    default: return 'Document No';
  }
};
...
export const calculateClosingBalance = (openingBalance, netAmount, paidAmount, transactionType) => {
  const multiplier = (transactionType === 'sale' || transactionType === 'debit_note') ? 1 : -1;
