...
const DownloadButton = ({ 
  ...
  const renderIcon = () => {
    if (status === 'completed') return <Check className="w-4 h-4 text-green-600" />;
        ...
        <div className="relative">
          <button
            className={`p-1.5 text-slate-500 hover:bg-white rounded-md transition-all shadow-sm flex items-center gap-1 ${
              disabled || isDownloading ? 'cursor-not-allowed opacity-80' : 'hover:text-indigo-600'
            }`}
            title={isDownloading ? `Downloading... ${Math.round(progress)}%` : "Download"}
            // Only toggle dropdown if NOT downloading and NOT disabled
            onClick={() => !isDownloading && !disabled && setDropdownOpen(!dropdownOpen)}
          ...
          >
            ...
            {!isDownloading && status === 'idle' && <ChevronDown className="w-3 h-3" />}
          </button>
