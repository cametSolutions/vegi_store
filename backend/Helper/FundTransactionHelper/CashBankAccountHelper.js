...
console.log("paymentMode",paymentMode)
  if (!cashAccount) {
    ...
    throw new Error('Cash account not found. Please create a Cash account (accountType: other, accountName: cash) for this branch.');
  }
 ...
 */
export const getBankAccount = async (companyId, branchId, bankAccountId,session) => {
  console.log("\n🏦 Finding Bank Account...");
 
  let bankAccount;

  if (bankAccountId) {
    // If specific bank account provided, use it
    ...
    if (!bankAccount) {
      throw new Error('Specified bank account not found or inactive');
    }
  } else {
    ...
    if (!bankAccount) {
      throw new Error('No active bank account found for this branch. Please specify bankAccount or create a bank account (accountType: other, accountName: [bank name]).');
    }
  }
...
export const getCashBankAccountForPayment = async ({
  ...
  console.log("\n🔍 Getting Cash/Bank account for payment mode:", paymentMode);
  ...
  if (isCashPayment) {
    ...
    const cashAccount = await getCashAccount(companyId, branchId, paymentMode,session);
    return {
    ...
    const bankAccount = await getBankAccount(companyId, branchId, bankAccountId, session);
    return {
 ...
 */
export const getAllCashBankAccounts = async (companyId, branchId,paymentMode) => {
