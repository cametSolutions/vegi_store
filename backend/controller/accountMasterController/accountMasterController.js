...
export const getallaccountHolder = async (req, res) => {
    try {
        const result = await AccountMasterModel.find({}).populate({ path: "pricelevel", select: "_id priceLevelName" })
        return res.status(201).json({ message: "account holders found", data: result })
    } catch (error) {
...
export const deleteAccntmaster = async (req, res) => {
    try {
        ...
        if (!deletedAccount) {
            ...
            const updatedaccntholder = await AccountMasterModel.find({ companyId, branchId })
...
export const updateAccntMaster = async (req, res) => {
    try {
        ...
        if (!updatedAccnt) {
            ...
            const updatedholerlist = await AccountMasterModel.find({}).populate({ path: "pricelevel", select: "priceLevel" })
