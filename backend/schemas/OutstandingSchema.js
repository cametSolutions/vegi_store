...
// ==================== INDEXES ====================
OutstandingSchema.index({ company: 1, outstandingType: 1, status: 1 });
OutstandingSchema.index({ account: 1, outstandingType: 1, status: 1 });
...
OutstandingSchema.index({ company: 1, branch: 1, outstandingType: 1 });
...
OutstandingSchema.methods.updateStatus = function(newStatus) {
    const validStatuses = ['pending', 'partial', 'paid', 'overdue', 'disputed', 'written_off'];
...
// Get overdue items
OutstandingSchema.statics.getOverdueItems = function(companyId, outstandingType = null) {
