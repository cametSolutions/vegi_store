...
export const CompanySchema = new mongoose.Schema(
  {
    ...
    mobile: {
      ...
      match: [/^[6-9]\d{9}$/, "Mobile number must be valid 10-digit Indian number"]
    },
...
CompanySchema.pre('save', function(next) {
  ...
  if (fyConfig) {
    ...
    if (this.isModified('financialYear.currentFY') && this.financialYear.currentFY) {
    ...
    this.financialYear.fyStartDate = new Date(Date.UTC(fyStartYear, fyConfig.start - 1, 1, 0, 0, 0));
    ...
    this.financialYear.fyEndDate = new Date(Date.UTC(fyEndYear, fyConfig.end - 1, lastDay, 23, 59, 59, 999));
  }
...
CompanySchema.methods.updateFYYear = function(newFYString) {
  if (!/^\d{4}-\d{2}$/.test(newFYString)) {
    throw new Error("Invalid FY format. Expected format: YYYY-YY (e.g., 2026-27)");
  }
